...
export const CommunicationOptionsSchema = Schema.Struct({
  readerModel: Schema.optionalKey(ModelReference),
});

export type CommunicationOptions = typeof CommunicationOptionsSchema.Type;
...
export const registerSessionCommunication = Effect.fn(
  "Academy.registerSessionCommunication"
)(function* registerSessionCommunication(
  ctx: Plugin.Context,
  options: CommunicationOptions
) {
  const scanRecords = Effect.fn("Academy.scanThreadRecords")(
      ...
      execute: (input) =>
        safe(
          scanRecords().pipe(
            Effect.map((records) => ({
              content: JSON.stringify(
                findThreadRecords(records, input.query ?? "", input.limit ?? 10)
              ),
            }))
          )
        ),
        ...
        safe(
          Effect.gen(function* readThread() {
            ...
            const result = yield* ctx.generate.text({
              model: Model.Ref.parse(options.readerModel ?? defaultReaderModel),
              prompt: [
      ...
      execute: (input) =>
        safe(
          ctx.session
            ...
            .pipe(Effect.map((result) => ({ content: JSON.stringify(result) })))
        ),
      input: ThreadInput,
